...
public class RelatorioPdfExporter {
    ...
    private void escreverCabecalho(Document documento, RelatorioDTO relatorio) throws DocumentException {
        documento.add(new Paragraph("EsteticaFlow - Relatorio gerencial", TITULO));
        documento.add(new Paragraph(relatorio.empresa() + " | Plano " + relatorio.plano().name(), TEXTO));
        var filtroTexto = relatorio.filtro() == null ? "-" : relatorio.filtro().name();
        documento.add(new Paragraph("Periodo: " + DATA.format(relatorio.periodo().inicio())
                + " a " + DATA.format(relatorio.periodo().fim())
                + " | Filtro: " + filtroTexto, TEXTO));
    ...
    private void escreverResumo(Document documento, RelatorioDTO relatorio) throws DocumentException {
        ...
        var tabela = tabela(5, "Receita", "Despesa", "Saldo", "Ticket medio", "Concluidos");
    ...
    private void escreverRankings(Document documento, RelatorioDTO relatorio) throws DocumentException {
        documento.add(new Paragraph(" "));
        documento.add(new Paragraph("Ranking de servicos", SUBTITULO));
        var servicos = tabela(3, "Servico", "Quantidade", "Valor");
    ...
    private void escreverDetalhes(Document documento, RelatorioDTO relatorio) throws DocumentException {
        ...
        var receitas = tabela(4, "Data", "Descricao", "Forma", "Valor");
        ...
        var despesas = tabela(4, "Data", "Descricao", "Categoria", "Valor");
        ...
        var agendamentos = tabela(6, "Data", "Cliente", "Veiculo", "Servicos", "Status", "Total");
